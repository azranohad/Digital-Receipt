...
import enchant
import datetime
from receiptData import itemObject
...
def clean_word(word):
    return word.replace('\'', '').replace('*', '').replace('%', '').replace('1LYDE', 'LIDL').replace('(','').replace(')','').replace('L+DI', 'LIDL')
...
MARKETS = ['drogerie', 'lidl', 'rewe', 'real', 'allguth', 'mels', 'morganfields']
...
class TesseractParser:
    def __init__(self, min_length=1, max_height=5, debug=False, iterations=1):
        ...
        self.dict_ger = enchant.Dict('en_US')
        self.debug = debug
    ...
    def get_meta_information(self, page):
        ...
        for line in lines[1:]:
            ...
            for fmt in ['%d.%m.%y', '%Y-%m-%d', '%d-%m-%Y','%d.%m.%y %H:%M', '%d/%m/%y', '%m/%d/%y']:
                try:
    ...
    def parse_page(self, page):
        ...
        return articles, total_price#, (0, 0, page.size[0], valid_bot)
